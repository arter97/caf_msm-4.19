...
static int _qcrypto_skcipher_init(struct crypto_skcipher *tfm)
{
	...
	struct qcrypto_alg *q_alg;
        ...
        if (alg == NULL || ctx == NULL) {
		pr_err("%s: Alg/ctx is NULL\n", __func__);
                return -1;
	}
	q_alg = container_of(alg, struct qcrypto_alg, cipher_alg);
	if( q_alg == NULL) {
		pr_err("%s: q_alg is NULL\n", __func__);
		return -1;
	}
		...
		unsigned int keylen)
{
	...
	if ((ctx->flags & QCRYPTO_CTX_USE_HW_KEY) == QCRYPTO_CTX_USE_HW_KEY) {
		pr_err(
		"%s HW KEY usage not supported for 3DES algorithm\n", __func__);
		return 0;
	}
...
static int _qcrypto_enc_aes_192_fallback(struct skcipher_request *req)
{
	...
	if( tfm == NULL || ctx == NULL) {
		pr_err("%s: tfm/ctx is NULL\n");\
		return -1;
	}
