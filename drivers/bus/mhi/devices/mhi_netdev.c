...
static unsigned char node_id[ETH_ALEN] = {0x02, 0x50, 0xf4, 0x00, 0x00, 0x00};
static const unsigned char default_modem_addr[ETH_ALEN] = {0x02, 0x50, 0xf3, 0x00, 0x00, 0x00};

#if defined(CONFIG_BRIDGE) || defined(CONFIG_BRIDGE_MODULE)
#define QCOM_BRIDGE_MODE
#endif

#define QCOM_RMNET_MODE
#define PCIE_MHI_DRIVER_VERSION "V1.3.0.13"
#define RANDOM_BYTES_LEN  3

#ifdef QCOM_BRIDGE_MODE
...
#define QCUSB_MRECEIVE_MAX_BUFFER_SIZE (1024*32)
...
static struct sk_buff * add_mbim_hdr(struct sk_buff *skb, u8 mux_id) {
	...
	__le32 sign;
	u8 *c;
	u16 tci = mux_id - QCOM_QMAP_MUX_ID;
...
static void rmnet_qmi_rx_handler(void *dev, struct sk_buff *skb_in)
{
	...
	while (skb_in->len > sizeof(struct qmap_hdr)) {
		...
		mux_id = map_header->mux_id - QCOM_QMAP_MUX_ID;
...
static int mhi_netdev_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (mhi_mbim_enabled) {
		if (add_mbim_hdr(skb, QCOM_QMAP_MUX_ID) == NULL) {
			dev_kfree_skb_any (skb);
			return NETDEV_TX_OK;
		}
	}
		...
		if (mhi_netdev->qmap_version == 5) {
			add_qhdr(skb, QCOM_QMAP_MUX_ID);
		}
		else if (mhi_netdev->qmap_version == 9) {
			add_qhdr_v5(skb, QCOM_QMAP_MUX_ID);
		}
		else {
			dev_kfree_skb_any (skb);
			return NETDEV_TX_OK;
...
static void mhi_netdev_setup(struct net_device *dev)
{
	...
	for(i = RANDOM_BYTES_LEN; i < ETH_ALEN; i++) {
		node_id[i] = addr[i];
	}
...
{
	mhi_netdev_debugfs_dentry = debugfs_create_dir(MHI_NETDEV_DRIVER_NAME, 0);
}
			    ...
			    const struct mhi_device_id *id)
{
	...
	if (ret) {
		pr_info("DTS does not have 'mhi,mru' for %s\n", id->chan);
	}
	...
	if (mhi_netdev->use_rmnet_usb) {
		...
		mhi_netdev->rmnet_info.mux_id[0] = QCOM_QMAP_MUX_ID;
#else
		unsigned i;

		for (i = 0; i < mhi_netdev->qmap_mode; i++) {
			u8 mux_id = QCOM_QMAP_MUX_ID+i;
