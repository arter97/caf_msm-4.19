				    ...
				    struct spi_miso *resp, int length)
{
	...
	if (resp->cmd == CMD_CAN_RECEIVE_FRAME) {
		...
		if (fw_resp->maj > 4 || (fw_resp->maj == 4 && fw_resp->min) ||
			...
			dev_info(&priv_data->spidev->dev, "checksum check Enabled\n");
			checksum_enable = 1;
		}
...
static ssize_t show_mcu_firmware(struct kobject *kobj,
	...
	/*
	 * MCU should be UP by now. If not, pass the default string
	 * to not break the UI.
	 */
	if (g_fw_str->maj != 0) {
...
static int qti_can_create_sysfs_entry() {
	...
	qti_fw_kobject = kobject_create_and_add("mcu_firmware", NULL);
	if (!qti_fw_kobject) {
		ret = -ENOMEM;
		return ret;
	}

	ret = sysfs_create_file(qti_fw_kobject, &fw_version_attr.attr);
	if (ret) {
		pr_err("Failed to create /sys/mcu_firmware file\n");
		ret = -1;
	}
	return ret;
}
...
static int qti_can_probe(struct spi_device *spi)
{
	...
	if (of_device_is_compatible(spi->dev.of_node, "qcom,nxp,mpc5746c"))
		...
		qti_can_bittiming_const = rh850_bittiming_const;
					 ...
					 "qcom,nxp,s32k144"))
		qti_can_bittiming_const = qti_can_data_bittiming_const;
	...
	if (!g_fw_str) {
		LOGDI("Memory alloc failure\n");
		err = -ENOMEM;
		goto unregister_candev;
	}

	/* Create sysfs entry for fw version */
	err = qti_can_create_sysfs_entry();
	if ((err == -1) || (err == -ENOMEM)) {
		LOGDE("Failed to create sysfs entry\n");
		err = -ENOMEM;
		goto unregister_candev;
	}
...
free_irq:
	...
	dev_info(dev, "unregister device\n");
