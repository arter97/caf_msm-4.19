...
#define ENTER      cnss_pr_dbg("Enter %s", __func__)
#define EXIT      cnss_pr_dbg("Exit %s", __func__)
				    ...
				    void *ss_handle)
{
	...
	if (driver_ops == NULL) {
		cnss_pr_err("Wlan device driver is not registerd.\n");
		return NOTIFY_OK;
	}
...
int cnss_resigter_driver_debug(struct cnss_plat_data *plat_priv)
{
	struct cnss_esoc_info *esoc_info;	ENTER;
	esoc_info = &plat_priv->esoc_info;
	esoc_info->modem_notify_handler =
			cnss_register_qcn9000_cb(plat_priv);
	return cnss_register_subsys(plat_priv);
}
...
static void m3_dump_open_timeout_func(struct timer_list *t)
{
	struct m3_dump *m3_dump_data = from_timer(m3_dump_data , t, open_timer);
	struct cnss_plat_data *plat_priv = container_of(m3_dump_data,
						struct cnss_plat_data, m3_dump_data);
...
static void m3_dump_read_timeout_func(struct timer_list *t)
{
	struct m3_dump *m3_dump_data = from_timer(m3_dump_data, t, read_timer);
	struct cnss_plat_data *plat_priv = container_of(m3_dump_data,
						struct cnss_plat_data, m3_dump_data);
...
int cnss_register_subsys(struct cnss_plat_data *plat_priv)
{
	...
	ENTER;
	subsys_info = &plat_priv->subsys_info;
	cnss_pr_err("register subsystem deviceID=0x%lx instanceID=0x%x\n",
							 plat_priv->device_id,
							 plat_priv->wlfw_service_instance_id);	
	switch (plat_priv->device_id) {
	case QCA6174_DEVICE_ID:
...
static int cnss_register_ramdump_v2(struct cnss_plat_data *plat_priv)
{
	...
	cnss_pr_dbg("Register subsys device %s\n", subsys_info->subsys_desc.name);

	if (!subsys_info->subsys_desc.name) {
		cnss_pr_err("subsystem not registerd\n");
		return 0;
	}
...
static int cnss_probe(struct platform_device *plat_dev)
{
	...
	cnss_pr_info("find a match device 0x%x", device_id->driver_data);
...
skip_soc_version_checks:
	...
	memset(plat_priv, sizeof(*plat_priv), 0);
